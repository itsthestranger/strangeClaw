...
class Tools:
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "max_results": {
                        "type": "integer",
                        "minimum": 1,
                        "description": "Optional result cap override for this query.",
                    },
                },
                "required": ["query"],
                "additionalProperties": False,
    ...
    def _web_fetch_schema() -> dict[str, Any]:
        return {
            "name": "web_fetch",
            "description": "Fetch a URL and extract readable content through the host broker.",
            "parameters": {
                "type": "object",
                "properties": {
                    "url": {"type": "string"},
    ...
    def _http_request_schema(self) -> dict[str, Any]:
        return {
            "name": "http_request",
            "description": (
                "Make a structured HTTP request through the host broker and return the response."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    ...
                    "integration": {
                        "type": ["string", "null"],
                        "description": (
                            "Named integration from secrets.yaml. Omit only for unauthenticated "
                            "public GET requests."
                        ),
                    },
                    "headers": {"type": "object"},
    ...
    def _execute_web_search(self, args: dict[str, Any]) -> ToolResult:
        ...
        default_max_results: Any = _DEFAULT_WEB_SEARCH_MAX_RESULTS
        if isinstance(web_search_cfg, dict):
            default_max_results = web_search_cfg.get(
        ...
        if broker is None:
            return ToolResult(
                exit_code=1,
                stdout="",
                stderr="web_search requires a host broker connection.",
            )
        ...
        return _broker_payload_to_tool_result(broker, payload)
    ...
    def _execute_web_fetch(self, args: dict[str, Any]) -> ToolResult:
        ...
        if broker is None:
            return ToolResult(
                exit_code=1,
                stdout="",
                stderr="web_fetch requires a host broker connection.",
            )

        payload = {"action": "web_fetch", "url": url.strip()}
        return _broker_payload_to_tool_result(broker, payload)
    ...
    def _execute_http_request(self, args: dict[str, Any]) -> ToolResult:
        ...
        if broker is None:
            return ToolResult(
                exit_code=1,
                stdout="",
                stderr="http_request requires a host broker connection.",
            )
...
def _broker_payload_to_tool_result(broker: BrokerClient, payload: dict[str, Any]) -> ToolResult:
    try:
        broker_result = broker.call(payload)
    ...
    if not isinstance(broker_result, dict):
        return ToolResult(
            exit_code=1,
            stdout="",
            stderr="Broker returned a non-object response payload.",
        )
    wrapped = _wrap_external_data(broker_result)
    success = broker_result.get("success") is True
    return ToolResult(exit_code=0 if success else 1, stdout=wrapped, stderr="")
